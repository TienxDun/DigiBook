  ...
  useEffect(() => {
    const fetchData = async () => {
      if (slug) {
        ...
        if (!foundBook) {
          foundBook = await db.getBookBySlug(slug);
        }
        ...
        if (foundBook) {
          ...
          const byAuthor = await db.getBooksByAuthor(bookData.author);
          setAuthorBooks(byAuthor.filter(b => b.id !== bookData.id).slice(0, 5));
