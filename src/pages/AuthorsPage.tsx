...
const AuthorsPage: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="bg-slate-50 min-h-screen pt-28 px-4 lg:px-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-12 h-48 bg-indigo-950 rounded-[2.5rem] flex flex-col justify-center px-10">
  ...
  return (
    <div className="bg-slate-50 min-h-screen pb-20 relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 lg:px-6 pt-20 lg:pt-28">
        ...
        {/* Header Block & Search */}
        <div className="bg-indigo-950 rounded-[2.5rem] lg:rounded-[3rem] p-6 lg:p-12 mb-10 relative overflow-hidden shadow-2xl shadow-indigo-950/20 border border-white/5">
           ...
           <div className="relative z-10 flex flex-col lg:flex-row items-end justify-between gap-10">
              <div className="flex-1">
                 <div className="flex items-center gap-3 mb-6">
                    ...
                    <span className="px-3 py-1 bg-indigo-500/20 text-indigo-300 rounded-lg text-micro font-bold uppercase tracking-premium border border-indigo-500/20">Cộng đồng sáng tác</span>
                 </div>
                 <h1 className="text-4xl lg:text-6xl font-extrabold text-white mb-6 tracking-tight leading-tight">Những người truyền <br className="hidden lg:block" /> cảm hứng</h1>
                 <p className="text-indigo-100/60 max-w-xl text-sm lg:text-base font-medium leading-relaxed mb-0">
                   Khám phá những cây bút lỗi lạc, những tâm hồn đồng điệu đang cùng DigiBook xây dựng một hệ sinh thái tri thức đa sắc màu.
                 </p>
              </div>
              ...
              <div className="w-full lg:w-96">
                 <div className="relative group">
                    <input 
                      ...
                      className="w-full py-5 pl-14 pr-6 bg-white/5 border border-white/10 rounded-2xl text-white outline-none focus:border-indigo-400 focus:bg-white/10 transition-all font-medium placeholder:text-indigo-200/30"
                    />
                 ...
                 <div className="mt-4 flex items-center justify-between px-2">
                    <p className="text-micro font-bold text-indigo-400 uppercase tracking-premium">Hiện có {authors.length} tác giả</p>
                    {searchQuery && (
                       <p className="text-micro font-bold text-indigo-300 uppercase tracking-premium">Tìm thấy {filteredAuthors.length}</p>
                    )}
                 </div>
              </div>
           </div>
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-12">
            {Object.keys(alphabeticalGroups).map(letter => (
              <button
                ...
                className="w-8 h-8 rounded-lg bg-white border border-slate-200 text-xs font-black text-slate-400 hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm"
              >
                {letter}
              </button>
            ))}
        ...
        {!searchQuery && (
          <div className="mb-14">
            <div className="flex items-center gap-4 mb-8 px-4">
               ...
               <h2 className="text-2xl font-black text-slate-800 uppercase tracking-premium">Tác giả nổi bật</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
              {featuredAuthors.map((author, index) => (
                <motion.div
                  ...
                  className="relative group bg-white p-6 rounded-[3rem] border border-slate-200/60 shadow-xl shadow-slate-200/50 hover:shadow-indigo-500/10 hover:border-indigo-200 transition-all duration-500"
                >
                  <Link to={`/author/${author.name}`} className="flex flex-col items-center">
                    <div className="w-32 h-32 rounded-3xl overflow-hidden mb-6 shadow-lg group-hover:scale-110 transition-transform duration-700 rotate-3 group-hover:rotate-0">
                      <img 
           ...
           {searchQuery ? (
                      ...
                      >
                        <Link to={`/author/${author.name}`} className="flex flex-col items-center">
                          <div className="relative mb-6">
                            <div className="relative w-32 h-32 lg:w-40 lg:h-40 rounded-[2.5rem] lg:rounded-[3rem] overflow-hidden border-[8px] border-white shadow-xl shadow-slate-200 group-hover:shadow-2xl group-hover:shadow-indigo-500/10 group-hover:border-indigo-50 transition-all duration-700 group-hover:-rotate-3 group-hover:scale-105">
                              <img 
                                src={author.avatar || `https://ui-avatars.com/api/?name=${author.name}&background=4f46e5&color=fff&size=512&bold=true`} 
                                alt={author.name} 
             ...
             Object.keys(alphabeticalGroups).map((letter) => (
               <div key={letter} id={`letter-${letter}`} className="relative scroll-mt-32">
                  <div className="flex items-center gap-6 mb-8 group/letter">
                     <span className="text-5xl font-black text-slate-200 group-hover/letter:text-indigo-100 transition-colors uppercase select-none">{letter}</span>
