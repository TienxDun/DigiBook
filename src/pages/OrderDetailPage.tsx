    ...
    <div className="bg-slate-50 min-h-screen pt-12 lg:pt-24 pb-32">
      ...
      <div className="max-w-6xl mx-auto px-4 lg:px-8">
        <motion.button 
          ...
          onClick={() => navigate('/my-orders')} 
          className="mb-10 flex items-center gap-2.5 text-[9px] font-black text-slate-400 hover:text-indigo-600 transition-all uppercase tracking-[0.2em]"
        >
              ...
              <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10 border-b border-slate-50 pb-8">
                <div>
                   ...
                   <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest flex items-center gap-2">
                      <i className="fa-regular fa-clock text-[9px]"></i> Thời gian đặt: {orderWithItems.date}
                   </p>
                </div>
                <div className={`px-5 py-2 rounded-xl font-black text-[9px] uppercase tracking-widest border shadow-sm ${
                  ...
                  <div className="flex justify-between relative z-10">
                    {steps.map((step, idx) => (
                      <div key={idx} className="flex flex-col items-center gap-3 flex-1">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all duration-700 relative ${
                          idx <= orderWithItems.statusStep ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-100' : 'bg-slate-50 text-slate-200'
                        }`}>
                        ...
                        <div className="text-center px-1">
                          <p className={`text-[9px] font-black uppercase tracking-widest mb-0.5 ${idx <= orderWithItems.statusStep ? 'text-indigo-600' : 'text-slate-400'}`}>
                            {step.label}
                          </p>
                          <p className="text-[7px] font-bold text-slate-300 leading-tight hidden sm:block uppercase tracking-wide">{step.desc}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="flex-grow min-w-0">
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-3 mb-3">
                         <div className="flex-grow min-w-0">
                            ...
                            <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-1.5">Giá: {formatPrice(item.priceAtPurchase)}</p>
                         </div>
                         <div className="flex flex-col items-end">
                            ...
                            <span className="text-[9px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg mt-1">SL: x{item.quantity}</span>
                         </div>
                      </div>
                      ...
                      <div className="flex items-center gap-3">
                         <Link 
                           to={`/book/${item.bookId}`} 
                           className="text-[8px] font-black text-indigo-600 hover:bg-indigo-600 hover:text-white px-3 py-1.5 border border-indigo-100 rounded-lg uppercase tracking-widest transition-all"
                         >
                            Mua lại
                         </Link>
                         <button className="text-[8px] font-black text-slate-300 hover:text-slate-900 uppercase tracking-widest transition-all">
                            Hỗ trợ
                         </button>
                      </div>
                    </div>
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
            ...
            >
              <h3 className="text-xs font-black text-slate-900 mb-6 flex items-center gap-2.5 uppercase tracking-widest relative z-10">
                <i className="fa-solid fa-map-location-dot text-indigo-500 text-[10px]"></i>
                Địa chỉ nhận hàng
              </h3>
              ...
              <div className="space-y-5 relative z-10">
                <div className="flex items-start gap-3.5">
                   ...
                   <div>
                      <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest mb-0.5">Người nhận</p>
                ...
                <div className="flex items-start gap-3.5">
                   <div className="w-9 h-9 rounded-xl bg-slate-50 flex items-center justify-center shrink-0 border border-slate-100">
                      <i className="fa-solid fa-truck text-[10px] text-slate-400"></i>
                   </div>
                   <div>
                      <p className="text-[8px] font-bold text-slate-300 uppercase tracking-widest mb-0.5">Địa chỉ giao</p>
                      <p className="font-medium text-slate-500 text-[11px] leading-relaxed">{orderWithItems.customer.address}</p>
                   </div>
                </div>

                {orderWithItems.customer.note && (
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100">
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest mb-1">Ghi chú:</p>
                    <p className="font-medium text-slate-600 text-[10px] leading-relaxed italic">
                       "{orderWithItems.customer.note}"
                    </p>
                  </div>
                )}
            ...
            >
              <h3 className="text-xs font-black mb-8 flex items-center gap-2.5 uppercase tracking-widest relative z-10">
                <i className="fa-solid fa-receipt text-indigo-400 text-[10px]"></i>
                Thanh toán
              </h3>
              ...
              <div className="space-y-4 mb-8 relative z-10">
                ...
                {orderWithItems.payment.couponDiscount > 0 && (
                  <div className="flex justify-between items-center text-[10px] font-bold uppercase tracking-widest text-emerald-400 px-1">
                    <span>Giảm giá</span>
                ...
                <div className="pt-6 border-t border-white/5 flex justify-between items-center">
                  <div className="flex flex-col">
                     <span className="text-[8px] font-black uppercase tracking-widest text-indigo-400 mb-1">TỔNG CỘNG</span>
              ...
              <div className="bg-white/5 rounded-xl p-3 mb-8 border border-white/5 text-center">
                 <p className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mb-0.5">Phương thức</p>
                 <p className="text-[10px] font-bold text-indigo-300">{orderWithItems.payment.method}</p>
              </div>

              <button 
                onClick={() => navigate('/')} 
                className="w-full py-4 bg-white text-slate-900 rounded-xl font-black text-[9px] uppercase tracking-[0.2em] hover:bg-slate-100 transition-all active:scale-95 relative z-10 mb-3"
              >
                 Tiếp tục mua sắm
              </button>
            </motion.div>
