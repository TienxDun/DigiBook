...
const OrderSuccess: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-6 py-24 lg:py-32 text-center fade-in max-w-2xl">
      ...
      <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-10">Mã đơn hàng: <span className="text-indigo-600">#{orderId.toUpperCase()}</span></p>
      ...
      <div className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm mb-10 text-left relative overflow-hidden">
        ...
        <h3 className="font-black text-[10px] uppercase tracking-widest text-slate-900 mb-4 flex items-center gap-2 relative z-10">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 px-4">
        <Link to="/" className="px-8 py-4 bg-slate-900 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-indigo-600 transition-all shadow-xl shadow-slate-200 active:scale-95">
          Tiếp tục mua sắm
        </Link>
        <Link to={`/my-orders/${orderId}`} className="px-8 py-4 bg-white text-slate-900 rounded-2xl font-black text-[10px] uppercase tracking-widest border border-slate-200 hover:bg-slate-50 transition-all text-center active:scale-95">
          Kiểm tra đơn hàng
        </Link>
      </div>
    </div>
