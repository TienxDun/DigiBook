      ...
      <div className={`${isMidnight ? 'bg-[#1e293b]/40 border-white/5' : 'bg-card/40 backdrop-blur-md border-border shadow-3xl'} flex flex-wrap items-center justify-between gap-4 p-6 rounded-[2.5rem] transition-all hover:border-primary/20`}>
        <div className="flex items-center gap-6">
          <div className="space-y-2">
            <span className="text-xs font-black uppercase tracking-premium text-muted-foreground ml-1">Vai trò</span>
            <div className={`flex gap-1 p-1 rounded-xl ${isMidnight ? 'bg-slate-700/30 border-white/10' : 'bg-secondary/30 border-border/50'
              }`}>
          ...
          <div className="space-y-2">
            <span className="text-xs font-black uppercase tracking-premium text-muted-foreground ml-1">Trạng thái</span>
            <div className={`flex gap-1 p-1 rounded-xl ${isMidnight ? 'bg-slate-700/30 border-white/10' : 'bg-secondary/30 border-border/50'
              }`}>
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto mt-1">
          <div className="relative group flex-1 sm:w-72">
            <i className="fa-solid fa-search absolute left-4 top-1/2 -translate-y-1/2 text-muted-foreground/50 group-focus-within:text-primary transition-colors text-sm"></i>
            <input
              type="text"
              placeholder="Tìm kiếm..."
              value={searchQuery}
              onChange={(e) => { setSearchQuery(e.target.value); setCurrentPage(1); }}
              className={`w-full pl-10 pr-4 py-3 rounded-xl border font-bold outline-none transition-all text-sm ${isMidnight
          ...
          <button
            ...
            title="Refresh"
          >
            <i className="fa-solid fa-rotate-right group-hover:rotate-180 transition-transform duration-500 text-lg"></i>
          </button>
        </div>
      </div>
                  ...
                  >
                      ...
                      <div className="flex items-center gap-4">
                        <div className="relative group/avatar">
                          <img
                            ...
                            alt={user.name}
                            className="w-12 h-12 rounded-xl object-cover shadow-md border-2 border-white transition-transform group-hover/avatar:scale-105"
                          ...
                          />
                          <div className={`absolute -bottom-0.5 -right-0.5 w-3.5 h-3.5 rounded-full border border-white shadow-sm transition-colors ${user.status === 'banned' ? 'bg-destructive' : 'bg-green-500 shadow-green-500/20'}`}></div>
                        </div>
                        <div>
                    ...
                    <td className="px-6 py-4 text-center">
                      <button
                        ...
                        className={`inline-flex items-center gap-2 px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all shadow-sm ${user.role === 'admin'
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-x-2 group-hover:translate-x-0">
                        <button
                          ...
                          className={`w-10 h-10 flex items-center justify-center rounded-lg transition-all shadow-sm ${(user.status || 'active') === 'active'
                        ...
                        <button
                          ...
                          className="w-10 h-10 flex items-center justify-center rounded-lg bg-secondary text-muted-foreground hover:bg-destructive hover:text-white transition-all shadow-sm active:scale-90"
                          title="Delete user"
                        >
                    ...
                    <td colSpan={5} className="px-6 py-12 text-center">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <div className="space-y-1">
                          ...
                          <p className="text-xs font-semibold text-muted-foreground/60">Try different filters or search terms.</p>
                        </div>
                        <button
