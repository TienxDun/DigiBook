...
const CategoryPage: React.FC<CategoryPageProps> = ({ onAddToCart }) => {
  ...
  const isPromotionPage = categoryName === 'Kinh tế' || categoryName?.toLowerCase().includes('sale') || categoryName?.toLowerCase().includes('khuyến mãi');
    ...
    <div className="min-h-screen bg-slate-50/50 pb-20" ref={topRef}>
      ...
      <div className="w-[92%] xl:w-[60%] mx-auto px-4 -mt-10 relative z-20">
        <div className="bg-white p-4 sm:p-6 rounded-[2rem] shadow-xl shadow-slate-200/50 border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-4 bg-slate-50 p-1.5 rounded-2xl border border-slate-100 w-full md:w-auto">
            <span className="text-[10px] font-black text-slate-400 pl-4 uppercase tracking-widest">Sắp xếp</span>
            <select 
              value={sortBy} 
              onChange={(e) => setSortBy(e.target.value)}
              className="bg-transparent text-sm font-black text-slate-900 outline-none pr-4 py-2 cursor-pointer flex-1 md:flex-none"
            >
