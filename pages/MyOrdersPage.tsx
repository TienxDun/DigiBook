  ...
  if (!user) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 text-center fade-in">
        ...
        <h1 className="text-2xl font-black text-slate-900 mb-2">Bạn chưa đăng nhập</h1>
        ...
        <button onClick={() => setShowLoginModal(true)} className="px-8 py-3.5 bg-indigo-600 text-white rounded-xl font-black text-sm shadow-xl shadow-indigo-100 active:scale-95 transition-all">Đăng nhập ngay</button>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 pt-20 lg:pt-24 pb-32 fade-in">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-indigo-600 mb-1.5">Lịch sử giao dịch</p>
      ...
      <div className="space-y-4">
        {orders.length > 0 ? (
              ...
              <div key={order.id} className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow group">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      <h3 className="text-base font-black text-slate-900">Đơn hàng #{order.id}</h3>
                      <p className="text-[11px] text-slate-400 font-bold">{order.date} • {itemCount} sản phẩm</p>
                    </div>
                  </div>

                  <div className="flex flex-wrap items-center gap-6 w-full md:w-auto">
                     <div>
                       <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-1">Trạng thái</p>
                       <span className={`px-3 py-1 rounded-lg text-[10px] font-black ${order.statusStep < 3 ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
                         {order.status}
                       </span>
                     </div>
                     <div>
                       <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-1">Tổng cộng</p>
                       <p className="text-base font-black text-slate-900">{formatPrice(order.payment.total)}</p>
                     </div>
                     <button onClick={() => navigate(`/my-orders/${order.id}`)} className="flex-1 md:flex-none px-5 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-black hover:bg-indigo-600 transition-all active:scale-95">Chi tiết đơn</button>
                  </div>
                </div>
              </div>
            );
          ...
          <div className="py-16 text-center bg-white rounded-[2.5rem] border-2 border-dashed border-slate-100">
            ...
            <h3 className="text-lg font-black text-slate-900 mb-1.5">Bạn chưa có đơn đặt hàng nào</h3>
            <p className="text-slate-500 text-xs mb-8 font-medium">Hãy trải nghiệm mua sắm những cuốn sách tuyệt vời ngay nhé!</p>
            <Link to="/" className="inline-block px-8 py-3.5 bg-indigo-600 text-white rounded-xl font-black text-sm active:scale-95 transition-all shadow-lg shadow-indigo-100">Mua sắm ngay</Link>
          </div>
        )}
      </div>
    </div>
