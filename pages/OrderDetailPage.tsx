...
const OrderDetailPage: React.FC = () => {
  ...
  if (!orderWithItems) {
    return (
      <div className="max-w-7xl mx-auto px-6 py-32 text-center fade-in">
        <h2 className="text-xl font-black text-slate-900 mb-4">Không tìm thấy đơn hàng #{orderId}</h2>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 pt-16 lg:pt-20 pb-32 fade-in">
      <button onClick={() => navigate('/my-orders')} className="mb-6 flex items-center gap-2 text-[11px] font-black text-slate-400 hover:text-indigo-600 transition-colors uppercase tracking-widest">
        ...
        <div className="lg:col-span-8">
          <div className="bg-white rounded-3xl border border-slate-100 p-6 lg:p-10 shadow-sm mb-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10 border-b border-slate-50 pb-6">
              <div>
                <h1 className="text-2xl font-black text-slate-900 mb-1">Đơn hàng #{orderWithItems.id}</h1>
                <p className="text-slate-400 font-bold text-[10px] uppercase tracking-wider">{orderWithItems.date}</p>
              </div>
              <div className="px-5 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-black text-[11px] uppercase tracking-wider">
              ...
              <div className="flex justify-between relative z-10 px-2 lg:px-4">
                {steps.map((step, idx) => (
                  <div key={idx} className="flex flex-col items-center gap-2.5">
                    ...
                    <span className={`text-[9px] font-black uppercase tracking-widest text-center ${idx <= orderWithItems.statusStep ? 'text-indigo-600' : 'text-slate-400'}`}>
                      {step.label}
                    </span>
                  </div>
                ))}
            ...
            <div className="space-y-6">
              <h2 className="text-lg font-black text-slate-900 mb-4">Sản phẩm trong đơn</h2>
                ...
                <div key={idx} className="flex gap-4 lg:gap-6 items-center border-b border-slate-50 pb-6 last:border-0 last:pb-0 group">
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      <Link to={`/book/${item.bookId}`} className="text-sm lg:text-base font-black text-slate-900 hover:text-indigo-600 transition-colors leading-snug">{item.title}</Link>
                      <span className="font-black text-slate-900 text-sm">{formatPrice(item.priceAtPurchase * item.quantity)}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] font-bold px-2 py-0.5 bg-slate-100 rounded text-slate-500 uppercase">Số lượng: {item.quantity}</span>
                      <Link to={`/book/${item.bookId}`} className="text-[10px] font-black text-indigo-600 hover:underline uppercase tracking-widest">Mua lại</Link>
                    </div>
                  </div>
                </div>
              ))}
        ...
        <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-24">
          <div className="bg-white rounded-3xl p-6 lg:p-8 border border-slate-100 shadow-sm">
            ...
            <div className="space-y-3.5">
              <div>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-0.5">Người nhận</p>
                <p className="font-black text-sm text-slate-900">{orderWithItems.customer.name}</p>
              </div>
              <div>
                <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest mb-0.5">Địa chỉ</p>
          ...
          <div className="bg-slate-900 text-white rounded-3xl p-6 lg:p-8 shadow-2xl relative overflow-hidden">
            ...
            <h3 className="text-base font-black mb-5 flex items-center gap-2.5 relative z-10">
              <i className="fa-solid fa-receipt text-indigo-400"></i>
              Thanh toán
            </h3>
            <div className="space-y-3.5 mb-6 relative z-10">
              <div className="flex justify-between text-xs">
                <span className="text-slate-500 font-bold uppercase tracking-widest">Tạm tính</span>
              ...
              <div className="pt-3.5 border-t border-white/10 flex justify-between items-center">
                ...
                <span className="text-xl font-black text-indigo-400">{formatPrice(orderWithItems.payment.total)}</span>
              </div>
            </div>
            <button onClick={() => navigate('/')} className="w-full py-3.5 bg-white text-slate-900 rounded-xl font-black text-[11px] uppercase tracking-widest hover:bg-indigo-50 transition-all shadow-lg active:scale-95 relative z-10">Tiếp tục mua sắm</button>
          </div>
        </div>
      </div>
    </div>
