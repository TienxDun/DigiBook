...
async function fetchBooks() {
    // Import node-fetch dynamically since native fetch might have some issues or differences, but let's just use native fetch
    ...
    const firstBook = Object.values(results)[0];
    if (firstBook) {
